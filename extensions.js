...
/* For the coffee shop - Appointment */

export const PersonInfoExtension = {
  name: 'PersonForms',
  type: 'response',
  match: ({ trace }) =>
    trace.type === 'person_ext_form' || trace.payload.name === 'person_ext_form',
...
export const MachineInfoExtension = {
  name: 'MachineForm',
  type: 'response',
  match: ({ trace }) =>
    trace.type === 'machine_ext_form' || trace.payload.name === 'machine_ext_form',
  render: ({ trace, element }) => {
    const formContainer = document.createElement('form')
    ...
    formContainer.innerHTML = `
          ...
          <input type="text" class="model" name="model" required><br><br>
    ...
    formContainer.addEventListener('submit', function (event) {
      ...
      window.voiceflow.chat.interact({
        type: 'complete',
        payload: { model: model.value, brand: brand.value },
      })
    })
